# goadserver.StatsPublisherApi

All URIs are relative to *https://up.go-adserver.com*

Method | HTTP request | Description
------------- | ------------- | -------------
[**api_v1_publisher_stats_get**](StatsPublisherApi.md#api_v1_publisher_stats_get) | **GET** /api/v1/publisher/stats | Publisher earnings (sites / adzones).


# **api_v1_publisher_stats_get**
> StatsResponse api_v1_publisher_stats_get(var_from=var_from, to=to, interval=interval, group_by=group_by, metrics=metrics, format=format, page=page, per_page=per_page, sort=sort)

Publisher earnings (sites / adzones).

**group_by:** `adzone`, `website`, `country`, `os`, `browser`,
`device`, `domain`, `subid`, `tag`.

**Filters:** `ad_type`, `website`, `adzone_id`, `selling_type`,
`device`, `country`, `tag_id`.

**Metrics:** `views`, `clicks`, `ctr`, `ecpm`, `earned`,
`un_views`, `no_ads`, `bot_views`, `bot_clicks`,
...
with goadserver.ApiClient(configuration) as api_client:
    ...
    api_instance = goadserver.StatsPublisherApi(api_client)
    ...
    try:
        # Publisher earnings (sites / adzones).
        api_response = api_instance.api_v1_publisher_stats_get(var_from=var_from, to=to, interval=interval, group_by=group_by, metrics=metrics, format=format, page=page, per_page=per_page, sort=sort)
        print("The response of StatsPublisherApi->api_v1_publisher_stats_get:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling StatsPublisherApi->api_v1_publisher_stats_get: %s\n" % e)
```
