# goadserver.StatsAdvertiserApi

All URIs are relative to *https://up.go-adserver.com*

Method | HTTP request | Description
------------- | ------------- | -------------
[**api_v1_advertiser_stats_get**](StatsAdvertiserApi.md#api_v1_advertiser_stats_get) | **GET** /api/v1/advertiser/stats | Display, native and CPM/RON campaign stats.


# **api_v1_advertiser_stats_get**
> StatsResponse api_v1_advertiser_stats_get(var_from=var_from, to=to, interval=interval, group_by=group_by, metrics=metrics, format=format, page=page, per_page=per_page, sort=sort)

Display, native and CPM/RON campaign stats.

**Available group_by dimensions:** `campaign`, `ad`, `adtype`,
`selling_type`, `adzone`, `website`, `publisher`, `device`,
`country`, `os`, `browser`, `domain`, `subid`.

**Filters (filter[…]):** `campaign_id`, `ad_type`, `selling_type`,
`device`, `country`, `adzone_id`, `site_id`.

**Metrics:** `views`, `clicks`, `ctr`, `ecpm`, `paid`, `conv_a`,
`conv_b`, `conv_c`.
...
with goadserver.ApiClient(configuration) as api_client:
    ...
    api_instance = goadserver.StatsAdvertiserApi(api_client)
    ...
    try:
        # Display, native and CPM/RON campaign stats.
        api_response = api_instance.api_v1_advertiser_stats_get(var_from=var_from, to=to, interval=interval, group_by=group_by, metrics=metrics, format=format, page=page, per_page=per_page, sort=sort)
        print("The response of StatsAdvertiserApi->api_v1_advertiser_stats_get:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling StatsAdvertiserApi->api_v1_advertiser_stats_get: %s\n" % e)
```
...
**429** | Per-key rate limit exceeded (default 60 rpm). |  -  |
