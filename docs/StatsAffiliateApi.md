# goadserver.StatsAffiliateApi

All URIs are relative to *https://up.go-adserver.com*

Method | HTTP request | Description
------------- | ------------- | -------------
[**api_v1_publisher_offers_stats_get**](StatsAffiliateApi.md#api_v1_publisher_offers_stats_get) | **GET** /api/v1/publisher/offers/stats | Affiliate / smartlink offer performance — publisher side.


# **api_v1_publisher_offers_stats_get**
> StatsResponse api_v1_publisher_offers_stats_get(var_from=var_from, to=to, interval=interval, group_by=group_by, metrics=metrics, format=format, page=page, per_page=per_page, sort=sort)

Affiliate / smartlink offer performance — publisher side.

**group_by:** `offer`, `country`, `device`, `os`, `browser`,
`domain`, `subid`, `website`, `adzone`, `sid1`, `sid2`, `sid3`,
`sid4`, `sid5`.

**Metrics:** `clicks`, `conversions`, `cr`, `epc`, `earned`,
...
with goadserver.ApiClient(configuration) as api_client:
    ...
    api_instance = goadserver.StatsAffiliateApi(api_client)
    ...
    try:
        # Affiliate / smartlink offer performance — publisher side.
        api_response = api_instance.api_v1_publisher_offers_stats_get(var_from=var_from, to=to, interval=interval, group_by=group_by, metrics=metrics, format=format, page=page, per_page=per_page, sort=sort)
        print("The response of StatsAffiliateApi->api_v1_publisher_offers_stats_get:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling StatsAffiliateApi->api_v1_publisher_offers_stats_get: %s\n" % e)
```
